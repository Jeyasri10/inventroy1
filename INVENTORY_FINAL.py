...
try:
    df['Date'] = pd.to_datetime(df['Date'])  # Update column name if different
...
if missing_cols:
    print(f"Error: Missing columns in feature_cols: {missing_cols}")
    exit()
...
# Drop Target & Non-Features (exclude Year, Month, Day)
...
numerical_cols = [col for col in numerical_cols if col in features.columns]  # Ensure columns exist
